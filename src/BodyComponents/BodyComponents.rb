...
class BodyComponents # {{{
      ...
      unless( @options.process.empty? )
        @options.process.each do |motion_config_file|
          ...
          if( @options.filter_motion_capture_data )
            message :info, "Filter Motion Capture data to smooth out outliers"
            filter_motion_capture_data @adt
          end 
  ...
  def clean # {{{
    message :success, "Cleaning up any old .gp, .gpdata and work/* files"
    `rm -f *.gp *.gpdata`
    `rm -f work/*.csv`
  end # of def clean }}}
  ...
  def parse_cmd_arguments( args ) # {{{
    ...
    opts                              = OptionParser.new do |opts|
      ...
      # Boolean switch.
      opts.on("-t", "--turning-pose-extraction", "Performs a turning pose extraction (CPA-PCA method)") do |t|
